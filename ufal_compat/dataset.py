...
class Dataset():
    def __init__(self, clusters):
        ...
        self.train_x = pd.read_csv(_os.path.join(_data, 'train_x.csv'))
        self.train_y = pd.read_csv(_os.path.join(_data, 'train_y.csv'))
        self.dev_x = pd.read_csv(_os.path.join(_data, 'dev_x.csv'))
        self.dev_y = pd.read_csv(_os.path.join(_data, 'dev_y.csv'))

        self.test_x = pd.read_csv(_os.path.join(_data, 'test_x.csv'))
        ...
        # Original UFAL setting: dev_x for published 0.755 DEV score
        self.train_dataset = self.create_dataset(pd.concat([self.train_y, self.dev_x]))
        # Alternative: test_x trains test embeddings inline (for test replication)
        # self.train_dataset = self.create_dataset(pd.concat([self.train_y, self.test_x]))
        
        self.all_features = np.array(self.all_features)
        self.class_weights = sklearn.utils.class_weight.compute_class_weight('balanced', classes=np.unique(self.all_features), y=self.all_features)
    ...
    def create_dataset(self, dataset):
        ...
        for line in dataset:
            ...
            new_dataset.append(np.array(new_line, dtype=object))
    ...
    def preprocess(self, dataset):
        ...
        drop_cols = [c for c in ['Unnamed: 0', 'index', 'wals_code', 'latitude', 'longitude', 'countrycodes'] if c in dataset.columns]
        return dataset.drop(columns=drop_cols)

    def batch_generator(self, batch_size=512):
        # Pre-build per-language sets for O(1) negative-sample rejection
        self._lang_pairs = [set(row[1:].tolist()) for row in self.train_dataset]
        while True:
            ...
            for idx in idxs:
                if np.random.uniform() < 0.5:
                    while True:
                        ...
                        if (column_id, feature_id) not in self._lang_pairs[idx]:
                            break
