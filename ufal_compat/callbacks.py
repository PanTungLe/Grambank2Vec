...
class KNN(tf.keras.callbacks.Callback):
    def __init__(self, neighbours):
        import os as _os
        _data = _os.path.join(_os.path.dirname(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__)))), 'data')
        self.neighbours = neighbours
        self.x = pd.read_csv(_os.path.join(_data, 'train_x.csv')).drop(columns=['Unnamed: 0', 'wals_code', 'latitude', 'longitude', 'countrycodes']).to_numpy()
        self.x_to_predict = pd.read_csv(_os.path.join(_data, 'dev_x.csv')).drop(columns=['Unnamed: 0', 'wals_code', 'latitude', 'longitude', 'countrycodes']).to_numpy()
        self.golden = pd.read_csv(_os.path.join(_data, 'dev_y.csv')).drop(columns=['Unnamed: 0', 'wals_code', 'latitude', 'longitude', 'countrycodes']).to_numpy()
...
class Filler(tf.keras.callbacks.Callback):
    def __init__(self, feature_maps, feature_maps_int):
        ...
        _data = _os.path.join(_os.path.dirname(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__)))), 'data')
        def _load(fname):
            df = pd.read_csv(_os.path.join(_data, fname))
            drop = [c for c in ['Unnamed: 0', 'index', 'wals_code', 'latitude', 'longitude', 'countrycodes'] if c in df.columns]
            return df.drop(columns=drop).to_numpy()
        self.x_to_predict = _load('dev_x.csv')
        self.golden = _load('dev_y.csv')
        self.test_x = _load('test_x.csv')
        self.feature_maps = feature_maps
        self.feature_maps_int = feature_maps_int

        self.best = 0

        self.form = pd.read_csv(_os.path.join(_data, 'dev_x.csv'))
    ...
    def fill_test(self):
        ...
        raw_test = pd.read_csv(_os.path.join(_data, 'test_x.csv'))
        test_cols = [c for c in ['Unnamed: 0', 'index'] if c in raw_test.columns]
        raw_np = raw_test.to_numpy()
        result = np.concatenate([raw_np[:, :8], tmp[:, 3:]], axis=1)
        test_col_names = list(raw_test.columns[:8]) + list(self.form.columns[8:len(tmp[0])])
        try:
            pd.DataFrame(data=result, columns=test_col_names).fillna('nan').to_csv('test_filled.csv', index=False)
            result2 = np.concatenate([raw_np[:, :8], fill_with_probs[:, 3:]], axis=1)
            pd.DataFrame(data=result2, columns=test_col_names).fillna('nan').to_csv('test_filled_probs.csv', index=False)
        except Exception as e:
            print(f'  [fill_test write skipped: {e}]')
